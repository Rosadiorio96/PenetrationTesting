...
class QLearningTable:
    def __init__(self, actions, learning_rate=0.1, reward_decay=0.99, e_greedy=1):
        # List of actions
    ...
    def choose_action(self, observation, episode):
        ...
        if random.uniform(0,1)>self.epsilon or episode > 150:
    ...
    def learn(self, state, action, reward, next_state):
        ...
        goal_ = next_state.replace("[", "")
        goal_ = goal_.replace("]", "")
        
        goal_ = " ".join(goal_.split())
        
        if goal_[len(goal_)-1] == '1' and goal_[len(goal_)-2] != '-':
